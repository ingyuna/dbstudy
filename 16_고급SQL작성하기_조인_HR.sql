...
SELECT
     ...
     , e.last_name
     , e.job_id
  FROM departments d INNER JOIN employees e  -- departments테이블이 driving 테이블, employees 테이블이 driven 테이블 
    ON d.department_id = e.department_id  -- 조건절의 등호(=) 왼쪽은 PK 도는 인덱스를 가진 칼럼이 좋다. 
